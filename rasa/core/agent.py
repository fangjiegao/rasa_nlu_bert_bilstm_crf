...
class Agent:
    ...
    async def parse_message_using_nlu_interpreter(
        # self, message_data: Text, tracker: DialogueStateTracker = None    # gfj
        self, message_data: Union[Text, UserMessage], tracker: DialogueStateTracker = None
    ) -> Dict[Text, Any]:
        ...
        # message = UserMessage(message_data)  # gfj

        if isinstance(message_data, Text):
    ...
    async def handle_message(
        ...
        **kwargs: Any,
        ...
        async with self.lock_store.lock(message.sender_id):
            res = await processor.handle_message(message)
            return res
